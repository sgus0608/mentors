...
public class FreeCommentDAO {
	...
	public ArrayList<CommentVO> findCommentList(long postNo) throws SQLException {
		...
		try {
			...
			sql.append("select c.comment_no, c.comment_content, ");
			sql.append("to_char(comment_time_posted,'YYYY.MM.DD HH24:MI:SS') as comment_time_posted, ");
			sql.append("m.nick_name, m.id ");
			sql.append("from free_comment c ");
