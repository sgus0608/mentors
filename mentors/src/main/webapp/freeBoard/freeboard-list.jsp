...
<table>
  ...
  <c:forEach items="${list}" var="list">
    <tr>
      ...
      <c:otherwise>
      <td><a href="FreeBoardPostDetailController.do?postNo=${list.postNo}">${list.title}</a></td>
      </c:otherwise>
      </c:choose>
...
<div class="row">
  ...
  <form class="form-inline" action="FreeBoardFindPostListController.do" method="get">
...
<ul class="pagination justify-content-center" style="margin:20px 0">
  <c:choose>
    ...
    <c:otherwise>
      <c:if test="${pagination.previousPageGroup}">
        <li class="page-item"><a class="page-link" href="FreeBoardFindPostListController.do?pageNo=${pagination.startPageOfPageGroup - 1}&category=${category}&searchText=${searchText}">Previous</a></li>
      </c:if>
      <c:forEach begin="${pagination.startPageOfPageGroup}" end="${pagination.endPageOfPageGroup}" var="page">
      <c:choose>
        <c:when test="${pagination.nowPage == page}">
          <li class="page-item active"><a class="page-link" href="FreeBoardFindPostListController.do?pageNo=${page}&category=${category}&searchText=${searchText}">${page}</a></li>
        </c:when>
      <c:otherwise>
        <li class="page-item"><a class="page-link" href="FreeBoardFindPostListController.do?pageNo=${page}&category=${category}&searchText=${searchText}">${page}</a></li>
      </c:otherwise>
      </c:choose>
      </c:forEach>
      <c:if test="${pagination.nextPageGroup}">
	    <li class="page-item"><a class="page-link" href="FreeBoardFindPostListController.do?pageNo=${pagination.endPageOfPageGroup + 1}&category=${category}&searchText=${searchText}">Next</a></li>
	  </c:if>
    </c:otherwise>
  </c:choose>
</ul>